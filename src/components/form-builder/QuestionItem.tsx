...
import { GripVertical, Trash2, Paperclip, Camera, Star, Heart, ThumbsUp, Circle, Square, Workflow } from 'lucide-react';
import { Question, FormSection } from '@/types/form';
...
const QuestionItem: React.FC<QuestionItemProps> = ({ 
  ...
  const handleLogicSave = (logic: any[]) => {
    onUpdate(sectionId, question.id, { conditionalLogic: logic });
  };
                ...
                >
                  ...
                  {question.conditionalLogic && question.conditionalLogic.length > 0 && (
                    <span className="ml-1 bg-purple-100 text-purple-800 text-xs px-1.5 py-0.5 rounded-full">
                      {question.conditionalLogic.length}
                    </span>
                  )}
                </Button>
              </div>
